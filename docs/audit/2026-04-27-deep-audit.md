...
#### P0.3 — Phase 1 Redis 多 worker【**audit correction 2026-04-27**: 實際已 active 在 production】

**原 finding**（基於 TODO.md row 44 標 `[D]` deployed-inactive，與真實 production 狀態不符）：
- ~~`.env` knob `OMNISIGHT_REDIS_URL=redis://ai_cache:6379/0` 沒 uncomment~~
- ~~`OMNISIGHT_WORKERS=1` 沒升到 2~~

**實際 production 狀態（2026-04-27 P0.3 follow-up 驗證）**：
- Production `.env`：`OMNISIGHT_REDIS_URL=redis://ai_cache:6379/0` ✅ active
- Production `.env`：`OMNISIGHT_WORKERS=2` ✅ active
- Container env 顯示兩 knob 都正確注入
- Process tree 顯示 uvicorn 跑 `--workers 2` + 兩 child multiprocessing.spawn process
- Redis (ai_cache) connection 從 backend container 可達（`redis-cli` ping success）

**結論**：Phase 1 已 active。TODO.md row 44 的 `[D]` 是 stale 紀錄、本日已修為 `[x]` + 加 verification annotation。
**Audit 自我反省**：依 TODO.md 紀錄判斷而非直接驗 production runtime → 產生 false-positive finding。下次 audit 應加「runtime 驗證」步驟給 deployed-inactive claim。

**P0.2 第 2 層（Phase 2 Ollama fallback）同樣 audit correction**：
- 原本 finding 寫「`.env` 4 個 knob 沒開」是 stale
- 實際 production：`OMNISIGHT_LLM_MODEL=claude-opus-4-7` + `OMNISIGHT_OLLAMA_BASE_URL=http://ai_engine:11434` + `OMNISIGHT_OLLAMA_MODEL=gemma4:e4b` + `OMNISIGHT_LLM_FALLBACK_CHAIN=anthropic,ollama` 都 active
- ai_engine 內 gemma4:e4b 模型 loaded、`/api/tags` 列得到
- fallback chain 真實可運作
- TODO.md row 45 已修為 `[x]` + verification annotation

**所以 P0.2 真正剩下的 only 是**：
- **第 1 層**：Anthropic console spend alert — 純 operator console 動作，AI 沒辦法做（仍是 P0）
- **第 3 層**：BP.F.8-F.10 hard-error classifier — Phase F 整個沒開工（仍是 P0、需 Phase B 先做、預期 1-2 月）
