...
- **註（FX2.D4.4 / OP-237, 2026-05-16）**：`OMNISIGHT_DECISION_BEARER`
  **已棄用**，下一個主版本將移除。請在 Admin UI → API Keys 簽發
  per-service 的 `omni_*` bearer（K6 方案）。env var 啟動時會自動
  遷移到 `api_keys` 列，遷移完成後即可移除環境變數。
