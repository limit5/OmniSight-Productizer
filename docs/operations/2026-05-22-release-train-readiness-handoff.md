...
5. **Run the staging gate** (smoke) → green JSONL evidence over the candidate digests (`scripts/staging_gate.py`).
   - ⚠ **Run gate AND promote from your NORMAL interactive shell — do NOT wrap them in `env -i PATH=/usr/bin:/bin:/usr/local/bin`** (the old clean-env habit). That habit was a stale workaround for a pydantic `extra_forbidden` raise in the gate's audit-DB write; it is fixed properly by exporting `OMNISIGHT_TOOLING_TOLERATE_EXTRA_ENV=1` (OP-1702, scoped to the gate/promote tooling — prod runtime stays strict). The clean env was actively harmful for promote: it stripped `cosign` (installed in `~/bin`, off the minimal PATH) so the inline `scripts/verify_image_signature.sh` cosign check exited 2 and aborted the promote mid-retag (v0.6.2 incident, OP-1736). cosign now self-locates (honours `$COSIGN_BIN`, else probes `~/bin` / `/usr/local/bin` / `~/go/bin`), but the rule stands: normal env + `OMNISIGHT_TOOLING_TOLERATE_EXTRA_ENV=1`, never `env -i`.
...
- **Deploy-overlay lock** (RT-08): `scripts/deploy-prod.sh --bundle <sealed> ...` is required for prod so `scripts/write_deploy_overlay_lock.py --bundle <sealed> --tag <tag> ... --out $OMNISIGHT_DEPLOY_OVERLAY_DIR/deploy-overlay.lock` writes the release SHA consumed by `/api/version` and `/api/v1/project-state`. Compose mounts `$OMNISIGHT_DEPLOY_OVERLAY_DIR:/etc/omnisight:ro`. Set `OMNISIGHT_REQUIRE_DEPLOY_OVERLAY=1` to make `/readyz` enforce it. A lock change needs the backend recreated (`up -d --force-recreate backend-a backend-b`, OP-1609) to be re-read.
...
- ⚠ **cosign discovery (OP-1736):** `scripts/verify_image_signature.sh` self-locates cosign — it honours `$COSIGN_BIN` if set, else probes `~/bin` / `/usr/local/bin` / `~/go/bin` and prepends the hit to PATH, and fail-closes (exit 2) with a `set COSIGN_BIN` hint if none is found. If cosign lives somewhere unusual, `export COSIGN_BIN=/path/to/cosign` rather than reaching for `env -i`/PATH surgery.
