---
id: ADR-0004
title: Per-agent JIRA identity (rejected shared bot pattern)
status: Accepted
date: 2026-05-04
---
...
Each agent will need to interact with the issue tracker (per [ADR 0001](ADR-0001-five-branch-gitflow.md) `feature/<JIRA-key>-<owner>-*` branch convention; per [ADR 0003](ADR-0003-gerrit-code-review.md) Gerrit + JIRA cross-link). Two identity models were evaluated:
...
The Gerrit dimension is decisive: [ADR 0003](ADR-0003-gerrit-code-review.md)'s submit-rule depends on identifying `merger-agent-bot` distinctly from other AI reviewers, and counting +1 votes per-agent. Shared bot model collapses all AI signal into one row.
...
- [ADR 0003 — Gerrit code review](ADR-0003-gerrit-code-review.md) — same identity model used for Gerrit per-reviewer +1/+2
