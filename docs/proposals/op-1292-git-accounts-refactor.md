# OP-1292 git accounts refactor proposal

## Context

OP-1292 names `backend/agents/git_accounts.py` as the primary refactor target.
Pre-flight verification found that path is absent in this checkout. The tracked
git-account modules are:

- `backend/git_accounts.py`
- `backend/routers/git_accounts.py`

Because the ticket explicitly scopes the proposal to
`backend/agents/git_accounts.py`, this pickup should not silently retarget the
...
1. `backend/git_accounts.py`, if OP-1292 meant the core git-account helper.
2. `backend/routers/git_accounts.py`, if OP-1292 meant the HTTP boundary.
3. A future `backend/agents/git_accounts.py`, if the missing agent module is
...
- No database, devops, embedded, frontend, security, tooling, or test-domain
  changes.
- No retargeting from `backend/agents/git_accounts.py` to another module without
  an explicit ticket update.

## Follow-up implementation ticket draft

Summary: Clarify OP-1292 git-account refactor target before implementation
...
Resolve the OP-1292 scope mismatch before any git-account refactor work:

- Confirm whether the intended target is `backend/git_accounts.py`,
  `backend/routers/git_accounts.py`, or a future
  `backend/agents/git_accounts.py`.
...
3. The corrected ticket preserves OP-1292's no-code-change proposal constraint
   or explicitly supersedes it with a new implementation ticket.
4. The original OP-1292 ticket is updated with the clarification outcome.
