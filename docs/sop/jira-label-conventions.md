...
| `area` | `area:<domain>` | enum `{backend, frontend, devops, tests, db, docs, security, embedded, tooling}` | **yes** | Prompt injection (`jira-ticket-conventions.md` §5): the runner injects `Areas: …` plus a *negation list* (the full `area:*` taxonomy minus the declared areas) into the agent prompt — the agent is told to stay strictly inside its declared areas, and to halt + surrender the ticket (§11) if the work needs an out-of-area domain. |
