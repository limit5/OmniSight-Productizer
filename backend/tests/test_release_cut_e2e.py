...
* the ``release-cut-promote`` submit-requirement, quadruply-keyed::

      applicableIf  = branch:main
                      AND topic:^release-v[0-9]+[.][0-9]+[.][0-9]+.*$
...
# ADR-0020 / OP-982 ``release-cut-promote`` submit-requirement parameters.
MERGER_GROUP = "merger-agent-bot"
HUMAN_GROUP = "non-ai-reviewer"
PROMOTE_AUTHORS = frozenset({"auto-promote-bot", "claude-bot", "codex-bot"})
RELEASE_TOPIC_RE = re.compile(r"^release-v[0-9]+[.][0-9]+[.][0-9]+.*$")
R3_FASTFORWARD_HASHTAG = "milestone:R3-fastforward"
...
class MockGerrit:
    ...
    def __init__(self, remote_repo: Path, *, author: str = "auto-promote-bot") -> None:
    ...
    def push_for_review(
        ...
        *,
        ...
        self.changes[number] = GerritChange(
            ...
            topic=topic, hashtags=tuple(hashtags), author=self.author,
    ...
    def _sr_applicable(change: GerritChange) -> bool:
        return (
            ...
            and change.topic is not None
            and RELEASE_TOPIC_RE.match(change.topic or "") is not None
            and R3_FASTFORWARD_HASHTAG in change.hashtags
            and change.author in PROMOTE_AUTHORS
        )
...
def _run_auto_promote(repo: Path, gerrit: MockGerrit) -> tuple[apm.PromotionResult, list, list, list]:
...
def test_release_cut_change_artifact_shape(tmp_path: Path) -> None:
    """AC #2 — one Gerrit change on ``refs/for/main`` carrying hashtag
    ``milestone:R3-fastforward`` + topic ``release-v0.5.0-rc2``, ``main``
    untouched until submit."""
    ...
    assert change.author == "auto-promote-bot"
    ...
    assert RELEASE_TOPIC_RE.match(change.topic) is not None
...
def test_release_cut_end_to_end_success(tmp_path: Path) -> None:
    """AC #1 / #5 — develop SHA → auto_promote_main → merge change → operator
    +2 → merger-bot conditional +2 → submit → main advances → bridge
    change-merged → release_audit row → release notification."""
    repo, remote = _repo_with_remote(tmp_path)
    main_before = _git(remote, "rev-parse", "main")
    develop_tip = _commit_file(repo, "feature.txt", "ship it\n")
    ...
    # (1) auto_promote_main produces exactly one merge change.
    result, _notifies, _events, audits = _run_auto_promote(repo, gerrit)
    assert result.status == "change_created"
    (number,) = gerrit.changes
    change = gerrit.changes[number]
...
def test_submit_requirement_not_applicable_without_r3_hashtag(tmp_path: Path) -> None:
    """Quad-key guard — strip the ``milestone:R3-fastforward`` hashtag and the
    release-cut-promote requirement stops being applicable; the merger-bot
    declines to vote and the change falls back to the human hard gate."""
    repo, remote = _repo_with_remote(tmp_path)
    _commit_file(repo, "feature.txt", "ship it\n")
    gerrit = MockGerrit(remote)
    _run_auto_promote(repo, gerrit)
    (number,) = gerrit.changes
    change = gerrit.changes[number]
    change.hashtags = tuple(h for h in change.hashtags if h != R3_FASTFORWARD_HASHTAG)

    assert MockGerrit._sr_applicable(change) is False
    assert gerrit.merger_bot_autovote(number) is False
    # even a double +2 from both groups can't satisfy the (now-applicable)
    # human gate without the non-ai-reviewer vote being the deciding one —
    # here we give only the merger vote to show the fallback still blocks.
    gerrit.review(number, value=2, group=MERGER_GROUP)
    assert gerrit.submittable(number) is False


def test_release_cut_promote_not_applicable_for_non_bot_author(tmp_path: Path) -> None:
    """Quad-key guard — a change pushed under a non-bot author can't ride the
    release-cut-promote bypass even with the right branch/topic/hashtag."""
    repo, remote = _repo_with_remote(tmp_path)
    _commit_file(repo, "feature.txt", "ship it\n")
    gerrit = MockGerrit(remote, author="rt3628")
    _run_auto_promote(repo, gerrit)
    (number,) = gerrit.changes
    change = gerrit.changes[number]

    assert change.author == "rt3628"
    assert MockGerrit._sr_applicable(change) is False
    assert gerrit.merger_bot_autovote(number) is False
