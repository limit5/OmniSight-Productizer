...
def test_total_re_export_count_pinned_at_192() -> None:
    ...
    adds 11 ``clone_audit`` symbols → 192. Each W11 row's drift guard
    is updated in lockstep so a future row that adds a new symbol fails
    every guard until each one acknowledges the new total."""
