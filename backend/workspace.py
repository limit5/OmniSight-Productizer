...
async def _install_sast_post_commit_hook(workspace_path: Path) -> None:
    """Install the SC.1.2 generated-workspace SAST post-commit hook.

    The hook exits 0 after recording the scanner result so SC.1.2 can
    trigger on every commit without implementing SC.1.3's PEP/HOLD gate.
    Module-global state audit: reads only immutable path constants; the
    generated hook derives scan state from each workspace's git HEAD.
    """
    rc, hook_path_raw, err = await _run(
        "git rev-parse --git-path hooks/post-commit",
        cwd=workspace_path,
    )
    if rc != 0 or not hook_path_raw.strip():
        logger.warning(
            "Could not resolve SAST post-commit hook path for %s: %s",
            workspace_path,
            err,
        )
        return

    hook_path = Path(hook_path_raw.strip())
    if not hook_path.is_absolute():
        hook_path = workspace_path / hook_path
    hook_path.parent.mkdir(parents=True, exist_ok=True)

    import shlex

    backend_root = shlex.quote(str(_MAIN_REPO.resolve()))
    hook_path.write_text(
        "#!/bin/sh\n"
        "# SC.1.2 -- Auto-generated by OmniSight workspace provisioning.\n"
        "commit_sha=\"$(git rev-parse HEAD 2>/dev/null || true)\"\n"
        f"OMNISIGHT_BACKEND_ROOT={backend_root}\n"
        "PYTHONPATH=\"$OMNISIGHT_BACKEND_ROOT${PYTHONPATH:+:$PYTHONPATH}\" "
        "python3 -m backend.security_scanning.sast "
        "--workspace \"$PWD\" --commit \"$commit_sha\" >/dev/null 2>&1 || true\n"
        "exit 0\n"
    )
    hook_path.chmod(0o755)


async def _install_secret_pre_commit_hook(workspace_path: Path) -> None:
    """Install the SC.5.2 generated-workspace secret pre-commit hook.

    The hook blocks commits only when the SC.5.1 scanner adapter reports
    blocking findings. Module-global state audit: reads only immutable
    path constants; the generated hook derives scan state from each
    workspace path and PATH-discovered scanner binaries.
    """
    rc, hook_path_raw, err = await _run(
    ...
    if rc != 0 or not hook_path_raw.strip():
        logger.warning(
            "Could not resolve secret pre-commit hook path for %s: %s",
            workspace_path,
            err,
        )
        return
    ...
    hook_path.write_text(
        "#!/bin/sh\n"
        "# SC.5.2 -- Auto-generated by OmniSight workspace provisioning.\n"
        f"OMNISIGHT_BACKEND_ROOT={backend_root}\n"
        "if ! PYTHONPATH=\"$OMNISIGHT_BACKEND_ROOT${PYTHONPATH:+:$PYTHONPATH}\" "
        "python3 -m backend.security_scanning.secrets "
        "--app-path \"$PWD\" >/dev/null; then\n"
        "  echo \"OmniSight secret scan blocked this commit.\" >&2\n"
        "  echo \"Run: python3 -m backend.security_scanning.secrets --app-path \\\"$PWD\\\"\" >&2\n"
        "  exit 1\n"
        "fi\n"
        "exit 0\n"
    )
...
async def provision(
    ...
    **Deprecation shim — ``repo_source=`` kwarg**: the legacy parameter
    ...
    await _install_sast_post_commit_hook(ws_path)
    await _install_secret_pre_commit_hook(ws_path)
...
async def finalize(agent_id: str) -> dict:
    ...
    info.commit_count += 1
    sast_scan: dict | None = None
    try:
        rc_head, head_sha, _ = await _run("git rev-parse HEAD", cwd=ws)
        effective_sha = head_sha.strip() if rc_head == 0 else ""
        from backend.security_scanning import (
            SAST_COMMIT_SCAN_ARTIFACT,
            scan_generated_workspace_commit,
        )

        artifact_path = ws / SAST_COMMIT_SCAN_ARTIFACT
        if artifact_path.exists():
            import json as _json

            artifact = _json.loads(artifact_path.read_text())
            if artifact.get("commit_sha") == effective_sha:
                sast_scan = artifact
        if sast_scan is None:
            scan = scan_generated_workspace_commit(ws, commit_sha=effective_sha)
            sast_scan = scan.to_dict()
        logger.info(
            "SAST commit scan for %s commit=%s source=%s passed=%s findings=%d",
            agent_id,
            str(sast_scan.get("commit_sha", ""))[:12],
            sast_scan.get("report", {}).get("source", ""),
            sast_scan.get("passed", False),
            sast_scan.get("report", {}).get("total_findings", 0),
        )
    except Exception as exc:
        logger.warning("SAST commit scan failed for %s: %s", agent_id, exc)
        sast_scan = {"triggered": False, "error": str(exc)}
