...
# Module-level registry keyed by task_id. A consume() pops out; a
# cancel_all() stops every container and clears the dict.
_prewarmed: dict[str, PrewarmSlot] = {}
_lock = asyncio.Lock()
...
async def prewarm_for(
    ...
    *,
    ...
    for t in candidates:
        agent_id = f"prewarm-{t.task_id}-{_short_id(dag.dag_id)}"
        async with _lock:
            if t.task_id in _prewarmed:
                logger.debug("prewarm: %s already slotted", t.task_id)
                out.append(_prewarmed[t.task_id])
                continue
        try:
            info = await starter(agent_id, workspace_path)
...
async def consume(task_id: str) -> Optional[PrewarmSlot]:
    """Pop and return the pre-warmed slot for `task_id`, or None on
    miss. Consumers MUST call this instead of inspecting _prewarmed
    directly — we need the lock + the metric bump."""
    async with _lock:
        slot = _prewarmed.pop(task_id, None)
    try:
        from backend import metrics as _m
...
async def cancel_all(*, stopper=None, reason: str = "dag_mutated") -> int:
...
def snapshot() -> dict[str, str]:
    """Read-only view of what's currently pre-warmed. Intended for
    `/healthz` and debug endpoints."""
    return {tid: slot.agent_id for tid, slot in _prewarmed.items()}
