    ...
    * **Task** — the user wants something executed. ``routed_to`` is
    ...
    if llm:
        sys = SystemMessage(content=(
            ...
            "   ALSO CONVERSATIONAL — anything about INSPECTING, CHECKING, VERIFYING, "
            "or RESCUING a JIRA ticket or the runner fleet: 'look at / check / "
            "verify / re-check ticket OP-XXXX', 'why is OP-XXXX stuck', 'fix / "
            "rescue / unwedge OP-XXXX', 'requeue / strip labels / comment on "
            "OP-XXXX', 'fleet / quota / incidents / deliveries'. The orchestrator "
            "(Sora) handles ALL of these herself with her own supervisor tools — "
            "NEVER route a ticket-inspection or rescue to a specialist (they'd "
            "resort to raw shell).\n"
            "2. Otherwise, when the user gives a direct one-shot CODE/BUILD execution "
            "command (build/compile/test/deploy the product now), decide which "
            "specialist agent should handle it. Valid agents: firmware, software, "
            "validator, reporter, reviewer, general. Respond with agent name(s) "
            "comma-separated (primary first).\n"
            "Examples:\n"
            "- 'What is ISP tuning?' → CONVERSATIONAL\n"
            "- 'How many agents are running?' → CONVERSATIONAL\n"
            "- '看一下 OP-2533 的狀態 / 再看一次 OP-2533 確認 stoploss 沒了' → CONVERSATIONAL\n"
            "- 'OP-2533 卡住了幫我修好，並留言' → CONVERSATIONAL\n"
            "- '現在車隊狀況怎麼樣' → CONVERSATIONAL\n"
...
async def conversation_node(state: GraphState) -> dict:
    ...
    persona += (
        ...
        "Only act when the operator asked you to fix/rescue "
        ...
        "unless the operator EXPLICITLY asks to. If you DO need to move status, "
        "call supervisor_list_transitions FIRST (workflows differ per project — "
        "there is often no plain 'Done'; closing can be multi-step), then use a "
        "real transition name. And always FINISH the steps the operator asked "
        "for (e.g. if they said 'leave a comment', do leave it) before you "
        "report back.\n"
