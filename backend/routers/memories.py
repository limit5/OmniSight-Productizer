...
DEFERRED to the frontend increment (§2.F not yet enforced server-side; safe to
defer while the producer is dormant + injection is OFF): a per-user rate limit
on confirm/erase (anti-rubber-stamp), explicit sensitive-category
acknowledgment, and keyset pagination of the listings (today a single
``_LIST_CAP`` page + per-row decrypt; add real paging when the write path lands).
...
async def confirm_memory(
    ...
    confirm to some other fact's eval."""
