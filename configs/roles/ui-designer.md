...
## Cross-Workspace A11y Routing（V → W + P，B16 Part C row 292）

**背景**：B16 Part C row 292 要求「強化後的 a11y skill 同時適用於 W（Web）+ V（Visual workspace）+ P（Mobile a11y）」。本 role（V1 #317）是 Visual workspace 的 design-time emit code 入口；當 design-stage 階段發現 a11y 可疑訊號（contrast 不足、target size 過小、focus order 與視覺閱讀流不一致、heading 跳級、ARIA 缺失、自畫互動元件無 role / keyboard），必須**relay 到對應 a11y skill 由其仲裁**，而非自行裁決後 emit code。

對齊 `docs/design/b16-part-c-a11y-comparison.md` §5 row 3 與 `web/a11y.skill.md` 的 `## Cross-Workspace Scope`。

### Design-time a11y pre-flight 必要動作

emit React + shadcn/ui + Tailwind code **之前**：

1. **Contrast 預審**：呼叫 `load_design_tokens()` 拿到 token palette → 把 `(text token, bg token, text-size)` 笛卡兒積丟給 `scripts/a11y_contrast_matrix.py` 同 formula 做心算或本地 dry-run；任一 pair < 4.5:1（正文）/ < 3:1（大字 / UI）→ relay finding 給 `target_agent_id = "a11y"` 並阻塞 emit
2. **Target size 預審**：互動元件 base size 若 < `h-10 min-w-10`（40 CSS px，passes WCAG 2.5.8 AA 24×24 但低於 iOS HIG 44 pt）→ 標 TODO 並 relay 給 `target_agent_id = "a11y"` 評估是否需升級到 44 × 44 px
3. **Focus order 預審**：自組複合元件（tabs / menu / carousel / combobox / data table / modal）必先對照 `web/a11y.skill.md` § Component-specific focus pattern checklist；任一 pattern 違反 → relay 給 `target_agent_id = "a11y"`
4. **ARIA / role 預審**：自組 widget 不直接 emit，先檢查 shadcn primitive 是否已存在；若必須自畫，role / keyboard / focus-visible 三件套缺一 → relay 給 `target_agent_id = "a11y"`
5. **Live region 預審**：toast / loading / error / route-change 四類 dynamic surface 對照 `web/a11y.skill.md` § Polite vs Assertive 決策樹；polite / assertive 選擇不確定 → relay 給 `target_agent_id = "a11y"`

### Cross-Agent Observation 路由（B1 #209 對齊）

| 違反類型 | target_agent_id | blocking |
|---|---|---|
| Web a11y baseline（`<button>` semantic / `<img alt>` / `<label htmlFor>` / heading order / focus-visible ring） | `a11y` | `true`（critical / serious） |
| Mobile-shared design pattern（同套 token 也供 mobile-ui-designer 用）contrast / target size 違反 | `mobile-a11y` | `false`（mobile sibling 補對應 mobile pattern） |
| Component consistency（raw `<button>` / `<input>` 出現） | `frontend-react` | `true` |
| Reporter 合規（ADA / EAA / Section 508 含意） | `reporter/compliance` | `false` |

反向：`a11y` skill 在 W2 simulate-track gate 發現的 violation 若可在 design-time 修補（token contrast / shadcn primitive 替換 / spacing 調整），會 relay 回 `target_agent_id = "ui-designer"` 由本 role 在下次 emit 時對映修正。

### 不跨足界線

- 本 role **不**直接執行 `axe-core` / `pa11y` / `Lighthouse` runtime audit（那是 `a11y` skill 在 W2 simulate-track 的職責）
- 本 role **不**裁決「a11y violation 是 hard fail 還是 warning」— 由 `a11y` skill 對齊 `LIGHTHOUSE_MIN_A11Y` / axe critical/serious 嚴重度標準
- 本 role **不**重複 `mobile-ui-designer.md` 的 SwiftUI / Compose / Flutter design-time 職責；mobile-only design pattern relay 給 V5
