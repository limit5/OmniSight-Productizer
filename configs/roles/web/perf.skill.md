...
1. **絕不**宣稱「優化了」卻沒附 before/after Lighthouse + CrUX / RUM 對比 — 「看起來順」不是評審標準，無量測 PR 一律退回
2. **絕不**用 p50 / median 當 CWV 達標證據 — CWV 官方門檻看 p75；只接受 `backend/observability/vitals.py` 的 RUM P75 數字
3. **絕不**追 FID 或在新程式碼 target FID — 2024-03-12 起已被 INP 取代，追 FID 是優化死指標
4. **絕不**讓 bundle 超過 profile `bundle_size_budget`（static 500 KiB / SSR-Node 5 MiB / CF 1 MiB / Vercel 50 MiB）— W2 `run_bundle_gate()` hard fail，不能靠「下 PR 再縮」延後
5. **絕不**讓單一 chunk 超過 `budget / 2` — 避免單一 chunk.js 吃掉整個預算；W2 heuristic 直接退
6. **絕不**讓 `<img>` / `<video>` / `<iframe>` 沒有 `width` + `height` 或 `aspect-ratio` CSS — 必 CLS，違者 hard fail
7. **絕不**用 `font-display: block` — long FOIT / FOUT 傷 LCP + CLS；一律 `swap`（body 字型可 `optional`），並加 `size-adjust`
8. **絕不**在 main thread 跑 `JSON.parse(hugePayload)` 或 > 50ms 的 long task — 必拆 `scheduler.yield()` / `requestIdleCallback` / Web Worker
9. **絕不**用 `setState` / reactive write 於 `onScroll` / `onResize` / `onMouseMove` 不 throttle — 必 INP 炸，至少 `requestAnimationFrame` 或 `useDeferredValue`
10. **絕不**同步 import 大型動畫 / 圖表庫（`lottie-web` / `chart.js` / `three.js` / `moment`）於首屏 — 一律 dynamic import + `Suspense` / `<ClientOnly>`
11. **絕不**讓第三方 script 以 blocking 方式載入 — `<script async>` / `<script defer>` / dynamic 三選一，blocking → hard fail
12. **絕不**在 PR description 省略 bundle-analyzer 截圖（`rollup-plugin-visualizer` / `@next/bundle-analyzer`）— diff > +5 KiB 必須附 reviewer 核准紀錄
13. **絕不**用 Brotli 壓縮級別的微調當 LCP 優化手段 — 砍 300 KB JS 永遠贏 Brotli level 6 → 11；從 source 下手，不是從壓縮
14. **絕不**讓 LCP element 沒有 `<link rel="preload" as="image" imagesrcset>` + `<img fetchpriority="high">` + CDN edge cache 三件套 — 未 preload → hard fail
