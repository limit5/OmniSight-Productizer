#!/usr/bin/env bash
# OP-1972 C4-D.C - Sign and checksum the MediaTek Genio 1200-EVK BSP bundle.
set -euo pipefail
...
usage() {
  cat <<'USAGE'
usage: sign_bsp_genio1200.sh --evk mediatek-genio1200 --version <version> --archive <bsp.tar.gz> --image <genio1200.wic.img> --out-dir <dir> --cosign-key <key> --public-key <cosign.pub> [options]

Creates the Case 4 MediaTek Genio 1200-EVK delivery manifest, SHA256SUMS,
cosign blob signatures, and verification log for the customer BSP bundle.

Options:
  --evk <id>          Required safety acknowledgement; must be mediatek-genio1200.
  --version <value>   BSP delivery version written into the manifest.
  --archive <path>    BSP archive to sign.
  --image <path>      Genio flash image to sign.
...
die() {
  printf 'sign_bsp_genio1200.sh: %s\n' "$*" >&2
  exit 2
}

log() {
  printf '[sign-bsp-genio1200] %s\n' "$*"
}
...
[[ "${EVK}" == "mediatek-genio1200" ]] || die "--evk must be mediatek-genio1200"
...
MANIFEST="${OUT_DIR}/mediatek-genio1200-delivery-manifest.json"
