...
- [ ] 域名可設定：`OMNISIGHT_DOMAIN=sora-dev.app` env 或腳本內 default
- [ ] 測試：在乾淨 WSL2 上跑一次全流程 → 容器啟動 + health pass + CF tunnel active + 瀏覽器開啟
- [ ] 預估：**1 day**（腳本已寫好，需測試 + 修邊界情況）

### L10. Pre-built Docker Images on GHCR (#337)
- [ ] GitHub Actions workflow `.github/workflows/docker-publish.yml`：tag push 時自動 build + push `ghcr.io/your-org/omnisight-backend:latest` + `ghcr.io/your-org/omnisight-frontend:latest`
- [ ] `docker-compose.prod.yml` 改為 `image:` 優先（有 registry image 就 pull，沒有才 local build）
- [ ] Multi-arch build（`linux/amd64` + `linux/arm64`）via `docker buildx`
- [ ] Image size 優化：backend < 500 MB / frontend < 200 MB
- [ ] 效果：首次部署從「本地 build 5-10 分鐘」→「pull 30 秒」
- [ ] 預估：**0.5 day**

### L11. 雲端一鍵佈署按鈕 (#338)
- [ ] DigitalOcean App Platform：`deploy/digitalocean/app.yaml` + README Deploy 按鈕
- [ ] Railway：`deploy/railway/railway.json` + README Deploy 按鈕
- [ ] Render：`deploy/render/render.yaml` + README Deploy 按鈕
- [ ] 每個平台定義 services（backend + frontend）+ env vars（from `.env.example`）+ build commands
- [ ] README.md 加 Deploy 按鈕 badges（one-click 跳轉到平台佈署頁）
- [ ] 預估：**1 day**
