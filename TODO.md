...
- **Phase 3**: G4 PostgreSQL cutover — **LIVE on PG 2026-04-20**
  ...
  - [ ] → [V]: operator live verify（3-P2 CF flip + 3-P6 rename + 3-P7 cleanup 合流測試 — fresh private window login + dashboard 驗 zero 429 / zero TypeError / zero bounce）; 接 24h observation window
  - [ ] **3-Runtime-v2 operator deploy** (#85/#70)：`docker compose -f docker-compose.prod.yml build backend-a backend-b`、rolling recreate（drain → recreate → healthy → 下一 replica）、`/readyz` 綠 + pool stats healthy、fresh private window login + multi-tab + SSE 流暢、24h 觀察窗 clean → flip deployed-observed
