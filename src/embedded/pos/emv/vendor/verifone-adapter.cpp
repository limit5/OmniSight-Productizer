...
#if defined(OMNISIGHT_VERIFONE_WITH_SDK)
#include <verifone_verixos.h>
#endif
...
constexpr char kVerifoneTracePrefix[] = "verifone:";
...
{
	return family == "PCI-PTS-L3" || family == "VX" || family == "MX" ||
	       family == "UX" || family == "Carbon";
}

} // namespace

class VerifoneAdapter::Impl {
public:
	explicit Impl(VerifoneAdapterConfig config) : config_(std::move(config))
	{
	}
	...
	PaymentTerminalStatus initialize()
	{
		if (!valid_config())
			return invalid_argument(
				&last_error_,
				"Verifone terminal id, merchant id, PED, and cryptogram generator are required");
		...
		if (tamper_locked_)
			return invalid_state(&last_error_,
					     "Verifone terminal is tamper locked");

#if defined(OMNISIGHT_VERIFONE_WITH_SDK)
		const int open_status = verifone_verixos_open(
			config_.terminal_id.c_str(), config_.merchant_id.c_str(),
			&terminal_);
		if (open_status != 0 || !terminal_)
			return sdk_error(open_status,
					 "VerixOS terminal open failed");
#endif
		...
		initialized_ = true;
		emit(PaymentTerminalStatus::kOk, "Verifone terminal initialized");
...
#if defined(OMNISIGHT_VERIFONE_WITH_SDK)
		if (terminal_) {
			const int close_status = verifone_verixos_close(terminal_);
			terminal_ = nullptr;
			if (close_status != 0)
				status = sdk_error(close_status,
						   "VerixOS terminal close failed");
		}
#endif

		initialized_ = false;
		emit(status, "Verifone terminal shutdown");
		return status;
	}
		...
		const PaymentTerminalKeyCeremony &ceremony)
	{
		if (!initialized_)
			return invalid_state(&last_error_,
					     "Verifone terminal is not initialized");
		if (tamper_locked_)
			return invalid_state(&last_error_,
					     "Verifone terminal is tamper locked");
		if (!ceremony.ped_key.slot)
			return invalid_argument(&last_error_,
						"Verifone key slot is required");
		if (!valid_family(ceremony.family_id))
			return invalid_argument(
				&last_error_,
				"Verifone key ceremony family is not supported");
		if (ceremony.ceremony_token.empty())
			return invalid_argument(
				&last_error_,
				"Verifone key ceremony token is required");

#if defined(OMNISIGHT_VERIFONE_WITH_SDK)
		if (ceremony.require_vendor_confirmation) {
			const int confirm_status = verifone_verixos_confirm_key_injection(
			...
			if (confirm_status != 0)
				return sdk_error(
						confirm_status,
						"VerixOS key ceremony confirmation failed");
		}
#endif

		const PedStatus ped_status = config_.ped->injectKey(ceremony.ped_key);
		if (ped_status != PedStatus::kOk) {
			last_error_ = "Verifone PED key injection failed: ";
			last_error_ += config_.ped->lastError();
			return map_ped_status(ped_status);
		}

		emit(PaymentTerminalStatus::kOk,
		     "Verifone family key ceremony completed");
		return PaymentTerminalStatus::kOk;
	}
					...
					PaymentTerminalResult *result)
	{
		...
		if (request.require_pin_block) {
			...
			if (ped_status != PedStatus::kOk) {
				last_error_ = "Verifone PED PIN capture failed: ";
				last_error_ += config_.ped->lastError();
				return map_ped_status(ped_status);
			}
		}
		...
		if (status != PaymentTerminalStatus::kOk) {
			last_error_ = "Verifone cryptogram generation failed: ";
			last_error_ += config_.cryptogram_generator->lastError();
			return status;
		}

		pending.vendor_trace = kVerifoneTracePrefix;
		...
		*result = std::move(pending);
		emit(PaymentTerminalStatus::kOk,
		     "Verifone authorization path completed");
		return PaymentTerminalStatus::kOk;
	}
		...
		const PaymentTerminalTamperEvent &event)
	{
		if (event.reason.empty())
			return invalid_argument(&last_error_,
						"Verifone tamper reason is required");

#if defined(OMNISIGHT_VERIFONE_WITH_SDK)
		const int tamper_status = verifone_verixos_report_tamper(
		...
		const PaymentTerminalRequest &request, ApduResponse *response)
	{
		if (request.card_interface == PaymentTerminalCardInterface::kContact) {
			...
			if (exchange_status != Iso7816Status::kOk) {
				last_error_ = "Verifone contact APDU failed: ";
				last_error_ += config_.contact_reader->lastError();
			}
		...
		if (!config_.contactless_reader)
			return unavailable(&last_error_,
					   "Verifone contactless reader is not configured");
		...
		if (activate_status != Iso14443Status::kOk) {
			last_error_ = "Verifone contactless activation failed: ";
		...
		if (exchange_status != Iso14443Status::kOk) {
			last_error_ = "Verifone contactless APDU failed: ";
			last_error_ += config_.contactless_reader->lastError();
		}
...
#if defined(OMNISIGHT_VERIFONE_WITH_SDK)
	VerifoneVerixTerminal *terminal_ = nullptr;
#endif
};

VerifoneAdapter::VerifoneAdapter(VerifoneAdapterConfig config)
	: impl_(std::make_unique<Impl>(std::move(config)))
{
}

VerifoneAdapter::~VerifoneAdapter() = default;

VerifoneAdapter::VerifoneAdapter(VerifoneAdapter &&) noexcept = default;

VerifoneAdapter &VerifoneAdapter::operator=(VerifoneAdapter &&) noexcept =
	default;

PaymentTerminalStatus VerifoneAdapter::initialize()
{
	return impl_->initialize();
}

PaymentTerminalStatus VerifoneAdapter::shutdown()
{
	return impl_->shutdown();
}

bool VerifoneAdapter::available() const
{
	return impl_->available();
}

PaymentTerminalStatus VerifoneAdapter::injectKey(
	const PaymentTerminalKeyCeremony &ceremony)
{
	return impl_->injectKey(ceremony);
}

PaymentTerminalStatus VerifoneAdapter::authorize(
	const PaymentTerminalRequest &request, PaymentTerminalResult *result)
{
	return impl_->authorize(request, result);
}

PaymentTerminalStatus VerifoneAdapter::handleTamper(
	const PaymentTerminalTamperEvent &event)
{
	return impl_->handleTamper(event);
}

void VerifoneAdapter::onEvent(PaymentTerminalEventCallback callback)
{
	impl_->onEvent(std::move(callback));
}

const std::string &VerifoneAdapter::lastError() const
{
...
{
	switch (status) {
	case PaymentTerminalStatus::kOk:
		return "ok";
	case PaymentTerminalStatus::kInvalidArgument:
		return "invalid-argument";
	case PaymentTerminalStatus::kUnavailable:
		return "unavailable";
	case PaymentTerminalStatus::kInvalidState:
		return "invalid-state";
	case PaymentTerminalStatus::kReaderError:
		return "reader-error";
	case PaymentTerminalStatus::kPedError:
		return "ped-error";
	case PaymentTerminalStatus::kCryptogramError:
		return "cryptogram-error";
	case PaymentTerminalStatus::kTamperDetected:
		return "tamper-detected";
	case PaymentTerminalStatus::kBackendError:
...
{
	switch (card_interface) {
	case PaymentTerminalCardInterface::kContact:
		return "contact";
	case PaymentTerminalCardInterface::kContactless:
		return "contactless";
	}
...
{
	switch (action) {
	case PaymentTerminalTamperAction::kReportOnly:
		return "report-only";
	case PaymentTerminalTamperAction::kLockTerminal:
		return "lock-terminal";
	case PaymentTerminalTamperAction::kZeroizeKeys:
		return "zeroize-keys";
	}
...
#if defined(OMNISIGHT_VERIFONE_ADAPTER_SMOKE_MAIN)
int main()
{
