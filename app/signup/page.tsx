...
function EmailVerifyCard({ email }: { email: string }) {
  ...
  // The localized body uses {email} interpolation but we want the
  // address itself to render with the foreground colour for emphasis,
  // so we split the localized string around the `{email}` token rather
  // than letting next-intl substitute it inline. The drift-guard
  // ensures all locales include the `{email}` token.
  const SENTINEL = "__OMNISIGHT_EMAIL_PLACEHOLDER__"
  const body = tSignup("verifyEmailBody", { email: SENTINEL })
  ...
  return (
    ...
    >
      ...
      >
        {tAuth("backToSignIn")} <ArrowRight size={14} />
      </a>
    </div>
  )
}
    ...
    <>
      ...
      >
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                title={tSignup("rollPassword")}
                aria-label={tSignup("rollPassword")}
              ...
              <button
                ...
                title={copied ? tSignup("copied") : tSignup("copyToClipboard")}
                aria-label={tSignup("copyPassword")}
                className="p-1 rounded text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-[var(--background)]/60"
              >
                {copied ? (
              ...
              >
                {showPassword ? tSignup("hide") : tSignup("show")}
              </button>
            </div>
          </div>
        ...
        <p className="font-mono text-[10px] text-[var(--muted-foreground)] text-center leading-relaxed">
          ...
          >
            <ArrowLeft size={10} /> {tSignup("signInLink")}
          </a>
        </p>
