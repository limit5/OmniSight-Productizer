...
- When completing a task:
  ...
  - If a generalisable lesson emerged: create a per-file source lesson under `docs/sop/lessons/` and run `scripts/build_lessons_index.py` to **validate** it — the generated index was retired, so the script writes nothing; it fails if the frontmatter is missing `id`/`ticket`/`title`/`date`/`tags` or the body is missing any of the four required sections `**Situation**` / `**Fix**` / `**Verification**` / `**Generalisation**` (matched literally, so a decorated heading like `**Generalisation — root cause**` does not count). There is no index artefact to commit alongside the lesson. Read canonical rendered lessons at `https://docs.sora-dev.app/lessons/` (vague entries like "be more careful" are auto-rejected per `docs/sop/jira-ticket-conventions.md` §14)
