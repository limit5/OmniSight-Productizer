#!/usr/bin/env bash
# OP-1971 C4-D.B - MediaTek Genio 1200-EVK BSP/SDK package builder.
#
# Usage:
#   deploy/case4/genio1200/bsp-package.sh \
#     --kernel <Image> \
#     --dtb <genio-1200-evk.dtb> \
...
#   case4-genio1200-bsp-sdk-<version>/
#     boot/Image
#     boot/genio-1200-evk.dtb
...
write_runbook() {
  local path="$1"
  cat >"$path" <<'EOF'
# MediaTek Genio 1200-EVK BSP/SDK Customer Runbook

## Contents

- `boot/Image` - Genio 1200 Linux kernel image.
- `boot/genio-1200-evk.dtb` - MediaTek Genio 1200-EVK device tree blob.
...
- `licenses/LICENSES.spdx.json` - SPDX package/license manifest.
- `CHECKSUMS.sha256` - payload integrity checksums.

## Bring-up

1. Flash the BSP image using the Phase 0 MediaTek flash flow for Genio 1200-EVK.
...
Customer-side acceptance requires a real MediaTek Genio 1200-EVK boot, visible
camera enumeration, a successful stitching pipeline run, and checksum/signature
verification of the delivered archive.
EOF
}
...
write_manifest() {
  ...
  cat >"$path" <<EOF
{
  "package": "case4-genio1200-bsp-sdk",
  "ticket": "OP-1971",
  "evk": "MediaTek Genio 1200-EVK",
  "soc": "MediaTek Genio 1200",
  "arch": "aarch64",
  "version": "$version",
  "artifacts": [
    {
...
script_dir="$repo_root/deploy/case4/genio1200"
...
output_dir="$repo_root/dist/case4/genio1200"
version=""
signing_key=""
...
mkdir -p "$output_dir"
workdir="$(mktemp -d)"
package_name="case4-genio1200-bsp-sdk-$version"
...
copy_mode "$dtb" "$package_root/boot/genio-1200-evk.dtb" 0644
...
dtb_sha="$(sha256_file "$package_root/boot/genio-1200-evk.dtb")"
