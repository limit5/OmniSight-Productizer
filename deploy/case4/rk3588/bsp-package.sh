#!/usr/bin/env bash
# OP-1949 C4-A.B - ATK-DLRK3588 BSP/SDK package builder.
#
# Usage:
#   deploy/case4/rk3588/bsp-package.sh \
#     --kernel <Image> \
...
#   case4-rk3588-bsp-sdk-<version>/
#     boot/Image
#     boot/atk-dlrk3588.dtb
...
write_runbook() {
  local path="$1"
  cat >"$path" <<'EOF'
# ATK-DLRK3588 BSP/SDK Customer Runbook

## Contents

- `boot/Image` - RK3588 Linux kernel image.
- `boot/atk-dlrk3588.dtb` - ATK-DLRK3588 device tree blob.
...
1. Flash the BSP image using the Phase 0 RK3588 flash flow for ATK-DLRK3588.
...
Customer-side acceptance requires a real ATK-DLRK3588 EVK boot, visible UVC
...
script_dir="$repo_root/deploy/case4/rk3588"
...
output_dir="$repo_root/dist/case4/rk3588"
version=""
signing_key=""
...
mkdir -p "$output_dir"
workdir="$(mktemp -d)"
package_name="case4-rk3588-bsp-sdk-$version"
...
copy_mode "$dtb" "$package_root/boot/atk-dlrk3588.dtb" 0644
...
dtb_sha="$(sha256_file "$package_root/boot/atk-dlrk3588.dtb")"
