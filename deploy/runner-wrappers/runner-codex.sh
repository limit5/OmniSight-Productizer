...
WORKTREE="${OMNISIGHT_CODEX_WORKTREE:-${BASE_DIR}/OmniSight-codex-worktree}"

if [[ "${INSTANCE_ID}" != "default" ]]; then
  WORKTREE="${OMNISIGHT_CODEX_WORKTREE:-${BASE_DIR}/OmniSight-codex-${INSTANCE_ID}-worktree}"
fi

mkdir -p "${LOG_DIR}"
LOG="${LOG_DIR}/codex-bot-${INSTANCE_ID}-$(date -u +%Y%m%dT%H%M%SZ).log"

set -a
. /home/user/.config/omnisight/audit-db.env 2>/dev/null || true
set +a

export OMNISIGHT_RUNNER_CLASS=subscription-codex
export OMNISIGHT_RUNNER_INSTANCE_ID="${INSTANCE_ID}"
export OMNISIGHT_CODEX_WORKTREE="${WORKTREE}"
...
while true; do
  ...
  rc="${PIPESTATUS[0]}"
  set -e
  echo "[launcher-codex-${INSTANCE_ID}] tick exit rc=${rc}; sleeping ${RESTART_SLEEP}s" | tee -a "${LOG}"
